...
const loadLeaderboard = async () => {
  try {
    ...
    dataInitialized.value = true // Mark data as initialized after first successful load
  } catch (error) {
...
<template>
  <div class="leaderboard-view flex flex-col max-w-[1200px]">
    ...
    <div v-if="!dataInitialized || isLoading" class="flex justify-center p-6">
    ...
    <div class="w-full" v-else>
      <TabView class="w-full" @tab-change="handleTabChange" :activeIndex="period === 'all' ? 0 : 1">
        <TabPanel header="All Time" value="all-time">
          <div v-if="predictionStore.leaderboard.length === 0" class="empty-container p-4">
        ...
        <TabPanel header="Current Week" value="weekly" :disabled="!predictionStore.currentContest">
          <div v-if="!predictionStore.currentContest" class="empty-container p-4">
